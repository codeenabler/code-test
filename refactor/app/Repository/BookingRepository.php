...
class BookingRepository extends BaseRepository
{
    ...
    {
        return $this->getJobs($user_id, function($data) {
            // is customer
            $data['jobs'] = $data['cuser']->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language', 'feedback')->whereIn('status', ['pending', 'assigned', 'started'])->orderBy('due', 'asc')->get();
        }, function($data) {
            // is translator
            $data['jobs'] = Job::getTranslatorJobs($cuser->id, 'new');
            $data['jobs'] = $jobs->pluck('jobs')->all();
            ...
            if ($data['jobs']) {
                foreach ($data['jobs'] as $jobitem) {
                    if ($jobitem->immediate == 'yes') {
                        $data['emergencyJobs'][] = $jobitem;
                    } else {
                        $data['noramlJobs'][] = $jobitem;
                    }
                }
                $data['noramlJobs'] = collect($data['noramlJobs'])->each(function ($item, $key) use ($user_id) {
                    $item['usercheck'] = Job::checkParticularJob($user_id, $item);
                })->sortBy('due')->all();
            }

            return $data->only('emergencyJobs', 'noramlJobs', 'cuser', 'usertype')->toArray();
        });
    }
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        return $this->getJobs($user_id, function ($data) {
            // is customer
            $data['jobs'] = $cuser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language', 'feedback', 'distance')->whereIn('status', ['completed', 'withdrawbefore24', 'withdrawafter24', 'timedout'])->orderBy('due', 'desc')->paginate(15);
            $data['pagenum'] = 0;
        }, function ($data) {
            // is translator
            $jobs_ids = Job::getTranslatorJobsHistoric($data['cuser']->id, 'historic', $data['pagenum']);
            $totaljobs = $jobs_ids->total();
            $data['numpages'] = ceil($totaljobs / 15);

            $data['jobs'] = $jobs_ids;
            $data['noramlJobs'] = $jobs_ids;
        }, function () use ($user_id) {
            // output
            return $data->only('emergencyJobs', 'noramlJobs', 'jobs', 'cuser', 'usertype', 'numpages', 'pagenum')->toArray();
        }, ['numpages' => 0, 'pagenum' => $pagenum]);
    }

    protected function getJobs($user_id, callable $isCustomer, callable $isTranslator, callable $response, $extra_data = [])
    {
        $data = collect([
        ...
        if ($data['cuser'] && $data['cuser']->is('customer')) {
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            if (!isset($data['from_language_id'])) {
                return $this->response('fail', 'Du måste fylla in alla fält', 'from_language_id');
            }
            if ($data['immediate'] == 'no') {
                if (isset($data['due_date']) && $data['due_date'] == '') {
                    return $this->response('fail', 'Du måste fylla in alla fält', 'due_date');
                }
                if (isset($data['due_time']) && $data['due_time'] == '') {
                    return $this->response('fail', 'Du måste fylla in alla fält', 'due_time');
                }
                if (!isset($data['customer_phone_type']) && !isset($data['customer_physical_type'])) {
                    return $this->response('fail', 'Du måste göra ett val här', 'customer_phone_type');
                }
                if (isset($data['duration']) && $data['duration'] == '') {
                    return $this->response('fail', 'Du måste fylla in alla fält', 'duration');
                }
            } else {
                if (isset($data['duration']) && $data['duration'] == '') {
                    return $this->response('fail', 'Du måste fylla in alla fält', 'duration');
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
    ...
    protected function response($status, $message, $field_name)
    {
        return [
            'status' => $status,
            'message' => $message,
